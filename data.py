import pandas as pd

def load_device_data():
    try:
        df = pd.read_csv('assistive_devices.csv', dtype={'項次': str})
        df = df.fillna("")
        return df
    except Exception:
        return None

DEMENTIA_ICD = ["290.0", "290.10", "290.11", "290.12", "290.13", "290.20", "290.21", "290.3", "290.40", "290.41", "290.42", "290.43", "290.8", "290.9", "294.0", "294.10", "294.11", "331.0", "331.1", "F01.50", "F01.51", "F02.80", "F02.81", "F03", "F03.9", "F03.90", "F03.91", "F04", "F05", "G30.0", "G30.1", "G30.8", "G30.9", "G31.0", "G31.09"]
INTELLECTUAL_ICD = ["317", "318.0", "318.1", "318.2", "319", "F70", "F71", "F72", "F73", "F78", "F79"]

# 通用規則定義 (防止 app.py 載入失敗)
RULE_SPEECH_STD = {"cat": "第三類(語障)", "icf": ["b310", "b320", "b330", "s320", "04"]}
RULE_VISION_STD = {"cat": "第二類(視障)", "icf": ["b210", "s220", "02"]}
RULE_HEARING_STD = {"cat": "第二類(聽障)", "icf": ["b230", "03"]}

# 16 大類判定地圖 - 根據圖片矩陣精確修正
SPECIAL_RULES_MAP = {
    "4-6": {
        "direct": ["b110.4", "09", "b235", "03", "b710a", "b710b", "b730a", "b730b", "b735", "b765", "s730", "s750", "s760", "05"], 
        "pairs": [{"icf": ["b117", "b122", "b140", "b144", "b147", "b152", "b160", "b164", "10"], "icd": DEMENTIA_ICD}]
    },
    "7-9": {
        "direct": ["b110.4", "09", "b235", "03", "b710a", "b710b", "b730a", "b730b", "b735", "b765", "s730", "s750", "s760", "05"], 
        "pairs": [{"icf": ["b117", "b122", "b140", "b144", "b147", "b152", "b160", "b164", "10"], "icd": DEMENTIA_ICD}]
    },
    "13": {
        "direct": ["10"], 
        "pairs": [{"icf": ["b117", "b140", "b144", "b147", "b152", "b164"], "icd": DEMENTIA_ICD}]
    },
    "42-48": {
        "direct": ["b110.4", "09", "b710a", "b710b", "b730a", "b730b", "b735", "b765", "s730", "s750", "s760", "05"], 
        "pairs": [{"icf": ["b117", "b122", "b140", "b144", "b147", "b152", "b160", "b164", "10"], "icd": DEMENTIA_ICD}]
    },
    "49-51": {
        "direct": ["b110.4", "09", "b710a", "b710b", "b730a", "b730b", "b735", "b765", "s730", "s750", "s760", "05"], 
        "pairs": [{"icf": ["b110", "b117", "b122", "b140", "b144", "b147", "b152", "b160", "b164", "10"], "icd": DEMENTIA_ICD}]
    },
    "52-53": {
        "direct": ["b110.4", "09", "b710a", "b710b", "b730a", "b730b", "b735", "b765", "s730", "s750", "s760", "05"], 
        "pairs": [{"icf": ["b110", "b117", "b122", "b140", "b144", "b147", "b152", "b160", "b164", "10"], "icd": DEMENTIA_ICD}]
    },
    "55-57": {
        "direct": ["b110.4", "09", "b710a", "b710b", "b730a", "b730b", "b735", "b765", "s730", "s750", "s760", "05"], 
        "pairs": [{"icf": ["b110", "b117", "b122", "b140", "b144", "b147", "b152", "b160", "b164", "10"], "icd": DEMENTIA_ICD}]
    },
    "91": {
        "direct": ["10"], 
        "pairs": [{"icf": ["b110", "b117", "b140", "b144", "b147", "b164"], "icd": DEMENTIA_ICD}]
    },
    "94-100": {
        "direct": ["b110.4", "09", "b710a", "b710b", "b730a", "b730b", "b735", "b765", "s730", "s750", "s760", "05"], 
        "pairs": [{"icf": ["b110", "b117", "b122", "b140", "b144", "b147", "b152", "b160", "b164", "10"], "icd": DEMENTIA_ICD}]
    },
    "110-113": {
        "direct": ["b110.4", "09", "b710a", "b710b", "b730a", "b730b", "b735", "b765", "s730", "s750", "s760", "05"], 
        "pairs": [{"icf": ["b117", "b122", "b140", "b144", "b147", "b152", "b160", "b164", "10"], "icd": DEMENTIA_ICD}]
    },
    "135-153": {
        "direct": ["b110.4", "09", "b710a", "b710b", "b730a", "b730b", "b735", "b765", "s730", "s750", "s760", "05"], 
        "pairs": [{"icf": ["b110", "b117", "b122", "b140", "b144", "b147", "b152", "b160", "b164", "10"], "icd": DEMENTIA_ICD}]
    },
    "154-162": {
        "direct": ["b110.4", "09", "b710a", "b710b", "b730a", "b730b", "b735", "b765", "s730", "s750", "s760", "05"], 
        "pairs": [{"icf": ["b110", "b117", "b122", "b140", "b144", "b147", "b152", "b160", "b164", "10"], "icd": DEMENTIA_ICD}]
    },
    "163-164": {
        "direct": ["b110.4", "09", "b235", "03", "b710a", "b710b", "b730a", "b730b", "b735", "b765", "s730", "s750", "s760", "05"], 
        "pairs": [{"icf": ["b110", "b117", "b122", "b140", "b144", "b147", "b152", "b160", "b164", "10"], "icd": DEMENTIA_ICD}]
    },
    "165": {
        "direct": ["b110.4", "09", "b235", "03", "b710a", "b710b", "b730a", "b730b", "b735", "b765", "s730", "s750", "s760", "05"], 
        "pairs": [{"icf": ["b117", "b122", "b140", "b144", "b147", "b152", "b160", "b164", "10"], "icd": DEMENTIA_ICD}]
    },
    "166-169": {
        "direct": ["b110.4", "09", "b235", "03", "b710a", "b710b", "b730a", "b730b", "b735", "b765", "s730", "s750", "s760", "05"], 
        "pairs": [{"icf": ["b110", "b117", "b122", "b140", "b144", "b147", "b152", "b160", "b164", "10"], "icd": DEMENTIA_ICD}]
    },
    "170-172": {
        "direct": ["b110.4", "09", "b235", "03", "b710a", "b710b", "b730a", "b730b", "b735", "b765", "s730", "s750", "s760", "05"], 
        "pairs": [
            {"icf": ["b117", "b122", "b140", "b144", "b147", "b160", "b164", "b16700", "b16710", "b16701", "b16711", "06"], "icd": INTELLECTUAL_ICD},
            {"icf": ["b117", "b122", "b140", "b144", "b147", "b152", "b160", "b164", "10"], "icd": DEMENTIA_ICD}
        ]
    }
}

def get_rule_key(item_id):
    try:
        idx = int(item_id)
        for key in SPECIAL_RULES_MAP.keys():
            if "-" in key:
                start, end = map(int, key.split("-"))
                if start <= idx <= end: return key
            elif int(key) == idx: return key
    except: pass
    return None
